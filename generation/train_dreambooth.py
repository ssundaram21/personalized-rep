...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        self.instance_images_path = [x for x in Path(instance_data_root).iterdir() if x.suffix[1:] in IMAGE_EXTENSIONS]
        assert len(self.instance_images_path) == 3
        print(f"USING {self.instance_images_path}")
...
def main(args):
    name = "_".join(f"dreambooth_{args.pretrained_model_name_or_path}_{args.learning_rate}_{args.max_train_steps}".split("/"))
    ...
    if accelerator.is_main_process:
        ...
        accelerator.init_trackers(args.wandb_name, config=tracker_config, init_kwargs=init_kwargs)
